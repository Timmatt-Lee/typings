...
inputField.addEventListener('keydown', (e) => {
  function inputFieldClass() {
    if ((e.key >= 'a' && e.key <= 'z') || (e.key === '\'' || e.key === ',' || e.key === '.' || e.key === ';')) {
      const inputWordSlice = inputField.value + e.key;
      const currentWordSlice = wordList[currentWord].slice(0, inputWordSlice.length);
      inputField.className = inputWordSlice === currentWordSlice ? '' : 'wrong';
    } else if (e.key === 'Backspace') {
      const inputWordSlice = e.ctrlKey ? '' : inputField.value.slice(0, inputField.value.length - 1);
      const currentWordSlice = wordList[currentWord].slice(0, inputWordSlice.length);
      inputField.className = inputWordSlice === currentWordSlice ? '' : 'wrong';
    } else if (e.key === ' ' && currentLang !== '繁體中文') {
      inputField.className = '';
    } else if (e.key === 'Enter' && currentLang === '繁體中文') {
      inputField.className = '';
    }
  }
  ...
  if ((e.key === ' ' && currentLang !== '繁體中文') || (e.key === 'Enter' && currentLang === '繁體中文')) {
    e.preventDefault();
    ...
    if (inputField.value !== '') {
      ...
      if (typingMode === 'time') {
        ...
        if (currentWordPosition.top < nextWordPosition.top) {
          for (let i = 0; i < currentWord + 1; i += 1) textDisplay.childNodes[i].style.display = 'none';
        }
      }
...
// Get cookies
if (getCookie('theme') === '') setTheme('light'); else setTheme(getCookie('theme'));
if (getCookie('language') === '') setLanguage('english'); else setLanguage(getCookie('language'));
if (getCookie('wordCount') === '') setWordCount(50); else setWordCount(getCookie('wordCount'));
if (getCookie('timeCount') === '') setTimeCount(60); else setTimeCount(getCookie('timeCount'));
if (getCookie('typingMode') === '') setTypingMode('wordcount'); else setTypingMode(getCookie('typingMode'));
if (getCookie('punctuation') === '') setPunctuation('false'); else setPunctuation(getCookie('punctuation'));
